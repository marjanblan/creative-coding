  ...
  // псевдослучайность фиксируем
  const SEED = 129;
  rnd.setSeed(SEED);
  const palette = ['#FF4B41', '#9745FF', '#72F5A4', '#FFC0F0', '#FFE64F'];
  
  // Один-единственный sketch
  const sketch = async () => {
    console.log('[sketch] create()');
    // ВНИМАНИЕ: путь от sketch.js к шрифту
    await loadFont('CoupeurCarve', './demo/assets/fonts/CoupeurCarve-SemiBold.otf', {
      weight: '100 900'
    });
    ...
    return ({ context: ctx, width, height, playhead }) => {
      ...
      const angle = playhead * Math.PI * 2; // плавная петля 0..2π
      ...
      for (let y = 0; y < rows; y++) {
        for (let x = 0; x < cols; x++) {
          ...
          // выбор формы тоже от шума (но со сдвигом), чтобы не прыгало на петле
  ...
  console.log('[sketch] before start');
  
  // Запуск через UMD canvas‑sketch, встраиваем в <canvas id="c">
  CS(sketch, {
    ...settings,
    canvas: document.getElementById('c'),
    context: '2d'
  }).then(() => console.log('[sketch] started'));