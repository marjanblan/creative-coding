 ...
 * Controls: window.__demo.play(), window.__demo.pause(); Space toggles.
 */
...
(async function main() {
  ...
  // timekeeping to ensure smooth resume after pause
  let elapsed = 0;                 // seconds accumulated
  let baseTime = performance.now(); // when current segment started (ms)
  ...
  function render(playhead) {
    ...
    const angle = playhead * Math.PI * 2; // seamless loop [0..1) -> [0..2π)
  ...
  window.__demo = {
    play() {
      if (playing) return;
      playing = true;
      baseTime = performance.now() - elapsed * 1000; // resume from same moment
      raf = requestAnimationFrame(tick);
    },
    pause() {
      if (!playing) return;
      ...
      // elapsed keeps last value so resume continues smoothly
    }
  };

  // wire UI buttons (index.html already adds hotkeys)
  document.getElementById('btnPlay')?.addEventListener('click', () => window.__demo.play());
  document.getElementById('btnPause')?.addEventListener('click', () => window.__demo.pause());
