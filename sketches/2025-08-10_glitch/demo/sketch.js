...
(async function main() {
  ...
  // аккуратный учёт времени, чтобы пауза/резюм не сбрасывали фазу
  let elapsed = 0;                   // накопленное время в секундах
  let baseTime = performance.now();  // отметка старта текущего сегмента
  ...
  function render(playhead) {
    ...
    // Как в твоём рефе: Issue 02 чёрным (на чёрном фоне — фактически невидим)
    ctx.fillStyle = '#000';
  ...
  // публичные контролы для index.html
  window.__demo = {
    play() {
      if (playing) return;
      playing = true;
